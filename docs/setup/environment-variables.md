...
| JWT | `JWT_SECRET` | Shared HMAC secret của Auth và Gateway; đổi secret làm token cũ vô hiệu |
...
OAuth2 có giá trị mock mặc định nên service có thể boot nhưng login Google/GitHub không hoạt động đến khi đặt credentials thật. `K6_USER_*` chỉ dùng performance script.
...
- `AI_SERVICE_URI` được Compose cấp trực tiếp cho Gateway, mặc định `http://host.docker.internal:8000`; biến này chưa có trong `.env.example` nhưng có thể thêm vào `.env`.
- Redis/Rabbit/Eureka/Config host được Compose override bằng tên container, không dùng localhost bên trong container.

## Biến có trong `.env.example` nhưng không điều khiển runtime chung

Shared Config đang cố định `spring.jpa.hibernate.ddl-auto=validate`, nên `JPA_DDL_AUTO=update` trong `.env.example` không override cấu hình đó. Tương tự, một số biến `JPA_SHOW_SQL`, `JPA_FORMAT_SQL`, `PORT`, `API_PREFIX` không được các file Compose/Config hiện tại tham chiếu. Không nên mô tả chúng là feature đang hoạt động chỉ vì có trong `.env.example`.
